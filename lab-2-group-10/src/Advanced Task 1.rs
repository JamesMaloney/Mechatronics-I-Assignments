...
impl<T> PinType<T>{
    ...
    fn read(&self) {
        match self.pin {
            ...
            Pin::Output(_) => { panic!("ERROR: Cannot read from an output pin!")}
            Pin::Unused(_) => { panic!("ERROR: Cannot read from an unused pin!")}
        }
    }

    fn write(&self) {
        match self.pin {
            Pin::Input(_) => { panic!("ERROR: Cannot write to an input pin!")}
            Pin::Output(_) => { println!("Writing...")}
            Pin::Unused(_) => { panic!("ERROR: Cannot write to an unused pin!")}
        }
    }
}
